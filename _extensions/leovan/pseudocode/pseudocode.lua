    ...
    CodeBlock = function(el)
      ...
      for k, v in pairs(options) do
        if string.match(k, "^html-") then
          ...
          data_attr_str = data_attr_str .. " " .. data_k .. '="' .. v .. '"'
        end
      end
      local id_str = ""
      if algorithm_id then
        id_str = ' id="' .. algorithm_id .. '"'
      ...
      return pandoc.RawBlock(
        "html",
        '<div class="pseudocode-container"' .. id_str .. data_attr_str .. '><pre class="pseudocode">' .. source_code .. '</pre></div>'
      )
    end,
  }
    ...
    Cite = function(el)
      ...
      for k, v in pairs(global_options.html_identifier_number_mapping) do
        if cite_text == "@" .. k then
          ...
          local link = pandoc.Link({ pandoc.Str(link_text) }, link_src, "", pandoc.Attr("", { "quarto-xref" }))
