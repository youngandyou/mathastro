...
[Cauchy Theorem](../Cauchy_Theorem/index.qmd#sec-cauchy's-theorem)은 $f(z)$가 analytic하고 $f'(z)$가 연속이어야 한다는 두 가지 조건이 필요했었다. 이때 $f'(z)$가 연속이라는 조건을 빼도 동일한 결론이 성립한다는 사실을 보인 정리가 Cauchy-Goursat Theorem이다. 
