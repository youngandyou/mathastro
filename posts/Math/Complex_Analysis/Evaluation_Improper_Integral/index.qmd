...
**(i)** $f$ is analytic on the upper half-plane except for finitely many isolated singular points $z_1, \dots, z_n$ with $\mathrm{Im}(z_i) > 0$,

**(ii)** $f$ is continuous on the real axis,

**(iii)** $\displaystyle \lim_{z \to \infty} z f(z) = 0$,

**(iv)** $\displaystyle \int_{-\infty}^\infty f(x) dx$ exists.
...
의 해를 가지고, 이 중 상반평면에 있는 해는 $c_0, c_1, c_2$이므로 함수 $f$는 이 세 개의 고립특이점을 제외하고 항상 해석적이고 실근은 존재하지 않으므로 실수축에서 연속이다. 또한 자명하게
...
$$\begin{align*}
\int_{-\infty}^\infty \frac{1}{x^6+1} dx = 2 \int_0^\infty \frac{1}{x^6+1} dx = 2I
\end{align*}$$
...
**(iv)** $\displaystyle \int_{-\infty}^\infty f(x) e^{iax} dx$ exists.
