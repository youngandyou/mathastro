...
Since we have chosen $\rho$ arbitrarily, it follows that $\vert f(z) \vert = \vert f(z_0) \vert$ in the neighborhood $\vert z - z_0 \vert < \varepsilon$. By [Theorem 2](../Analytic_Function/index.qmd#sec-theorem-2), $f(z)$ is constant throughout that neighborhood, which means that $f(z) = f(z_0)$ for each point $z$ in the neighborhood. $\blacksquare$
...
즉 최대값은 항상 경계에서 일어날 수 밖에 없음을 보장해준다.
