...
# Theorem 5
**(i)** Let $A$ be a subset of a space $X$. If there exists a sequence of points of $A$ converging to $x \in X$, then $x \in \overline{A}$. Moreover, if $X$ is metrizable, then the converse holds.

**(ii)** Let $f: X \to Y$ be a continuous function. Then for every sequence $\{ x_n \} _{n=1} ^{\infty}$ in $X$ with $\lim_{n \to \infty} x_n = x$, we have $\lim_{n \to \infty} f(x_n) = f(x)$. Moreover, if $X$ is metrizable, then the converse holds. 
...
# Theorem 6
Let $\mathbb{R}^n$ be the metric space with the Euclidean metric $d$. A subspace $A$ of $\mathbb{R}^n$ is [compact](../Compact_Space/index.qmd#sec-definition) $\iff$ $A$ is [closed](../Closed_Set/index.qmd#sec-definition) and bounded.

## Proof
$(\implies)$ Suppose that $A$ is compact. [Since $(\mathbb{R}^n, d)$ is Hausdorff by Theorem 3, $A$ is closed.](../Compact_Space/index.qmd#sec-theorem) Consider the open cover $\{ B_d(\mathbf{0}, n) \mid n \in \mathbb{N} \}$ of $A$. Since $A$ is compact, there is a finite subcover $\{ B_d(\mathbf{0}, n_1), \dots, B_d(\mathbf{0}, n_k) \}$ of $A$ for some $n_1, \dots, n_k \in \mathbb{N}$. Let $M = max \{ n_1, \dots, n_k \}$. Then 
